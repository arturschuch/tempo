 ...
 * ```
 *
 * 1 is a parent of 2 and 5, 2 is a parent of 3, etc.
 * Note that depth is equal to the number of hyphens for each node.
 *
...
interface Hierarchy {
     ...
     * The depth for this node will be `depth(index)`.
     ...
     * The unique ID for this node will be `nodeId(index)`.
     * @param index must be non-negative and less than [size]
     */
...
fun Hierarchy.filter(nodeIdPredicate: (Int) -> Boolean): Hierarchy {
    ...
    val filteredNodeIds = ArrayList<Int>(size)
    val filteredDepths = ArrayList<Int>(size)

    val keepAtDepth = BooleanArray(size + 1)

    for (i in 0 until size) {
        val currentDepth = depth(i)
        val parentKept = if (currentDepth == 0) true else keepAtDepth[currentDepth - 1]
        val shouldKeep = parentKept && nodeIdPredicate(nodeId(i))

        keepAtDepth[currentDepth] = shouldKeep
